...
const { FakeObserver, mockRideService, mockWorkoutListService, mockRideDisplayService } = vi.hoisted(() => {
    class FakeObserver {
    ...
    const mockRideDisplayService = {
        isOverlayPinned: vi.fn(() => false),
        setOverlayPinned: vi.fn(),
    }

    return { FakeObserver, mockRideService, mockWorkoutListService, mockRideDisplayService }
})
...
vi.mock('incyclist-services', async (importOriginal) => {
    ...
    return {
        ...
        useRideDisplay: () => mockRideDisplayService,
    }
})
...
describe('WorkoutControl pin persistence', () => {
    ...
    beforeEach(() => {
        mockRideDisplayService.isOverlayPinned.mockReturnValue(false)
    ...
    test('reads the persisted pinned state for the "workout-control" overlay on mount', async () => {
        renderControl()
        expect(await screen.findByText('ERG')).toBeInTheDocument()

        expect(mockRideDisplayService.isOverlayPinned).toHaveBeenCalledWith('workout-control')
    })

    test('renders already pinned when the service reports it as pinned', async () => {
        mockRideDisplayService.isOverlayPinned.mockReturnValue(true)
        renderControl()
        expect(await screen.findByText('ERG')).toBeInTheDocument()
    ...
    test('persists the pinned state via the service when the user pins the overlay', async () => {
        ...
        expect(mockRideDisplayService.setOverlayPinned).toHaveBeenCalledWith('workout-control', true)
    })
    ...
    test('persists the pinned state via the service when the user unpins the overlay', async () => {
        ...
        expect(mockRideDisplayService.setOverlayPinned).toHaveBeenCalledWith('workout-control', false)
    })
})
